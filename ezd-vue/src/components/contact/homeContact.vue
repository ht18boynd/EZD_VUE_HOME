<template>
  <startHeader></startHeader>
  <div class="cs-height_90 cs-height_lg_80"></div>
  <div class="cs-height_80 cs-height_lg_80"></div>
  <div class="container">
    <div class="row">
      <div class="col-lg-8 offset-lg-2">
        <div class="cs-contact_card_wrap">
          <div class="cs-contact_card">
            <div class="cs-contact_info text-center">
              <div class="cs-contact_icon">
                <svg
                  width="39"
                  height="39"
                  viewBox="0 0 39 39"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clip-path="url(#clip0_1448_14685)">
                    <path
                      d="M16.7113 22.6393C20.4669 26.4044 24.8179 29.9986 26.5422 28.2759C29.0074 25.8059 30.5274 23.6637 35.9646 28.0305C41.4018 32.3974 37.2249 35.3107 34.8373 37.6984C32.0823 40.4534 21.8128 37.8424 11.6573 27.6917C1.50177 17.5409 -1.10439 7.26986 1.65061 4.51169C4.04461 2.12878 6.95002 -2.04172 11.3169 3.38594C15.6837 8.81361 13.5383 10.3368 11.0746 12.8004C9.35194 14.5326 12.9461 18.882 16.7113 22.6393Z"
                      fill="url(#paint0_linear_1448_14685)"
                    />
                  </g>
                  <defs>
                    <linearGradient
                      id="paint0_linear_1448_14685"
                      x1="38.6309"
                      y1="0.724609"
                      x2="-6.93527"
                      y2="23.7985"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop offset="0" stop-color="#FC466B" />
                      <stop offset="1" stop-color="#3F5EFB" />
                    </linearGradient>
                    <clipPath id="clip0_1448_14685">
                      <rect
                        width="38"
                        height="38"
                        fill="white"
                        transform="matrix(-1 0 0 1 38.623 0.726074)"
                      />
                    </clipPath>
                  </defs>
                </svg>
              </div>
              <h3 class="cs-contact_title">Phone</h3>
              <p class="cs-contact_text">+7 (903) 679-96-15</p>
            </div>
            <div class="cs-contact_info text-center">
              <div class="cs-contact_icon">
                <svg
                  width="40"
                  height="39"
                  viewBox="0 0 40 39"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M35.8288 14.9442V29.2259C35.8288 30.9676 34.4038 32.3926 32.6621 32.3926H7.32878C5.58711 32.3926 4.16211 30.9676 4.16211 29.2259V10.2259C4.16211 8.48424 5.58711 7.05924 7.32878 7.05924H23.3204C23.2254 7.56591 23.1621 8.10424 23.1621 8.64258C23.1621 10.9859 24.1913 13.0601 25.8063 14.5167L19.9954 18.1426L7.32878 10.2259V13.3926L19.9954 21.3092L28.3871 16.0526C29.2421 16.3692 30.1288 16.5592 31.0788 16.5592C32.8679 16.5592 34.4988 15.9417 35.8288 14.9442ZM26.3288 8.64258C26.3288 11.2709 28.4504 13.3926 31.0788 13.3926C33.7071 13.3926 35.8288 11.2709 35.8288 8.64258C35.8288 6.01424 33.7071 3.89258 31.0788 3.89258C28.4504 3.89258 26.3288 6.01424 26.3288 8.64258Z"
                    fill="url(#paint0_linear_1448_14689)"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1448_14689"
                      x1="4.16211"
                      y1="3.89258"
                      x2="40.3952"
                      y2="24.2752"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop offset="0" stop-color="#FC466B" />
                      <stop offset="1" stop-color="#3F5EFB" />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
              <h3 class="cs-contact_title">Email</h3>
              <p class="cs-contact_text">kenzi.law@example.com</p>
            </div>
            <div class="cs-contact_info text-center">
              <div class="cs-contact_icon">
                <svg
                  width="41"
                  height="39"
                  viewBox="0 0 41 39"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M20.5552 0.543945C29.1928 0.543945 36.1943 7.54547 36.1943 16.1831C36.1943 22.7916 31.5507 29.9937 22.3837 37.8662C21.874 38.304 21.2241 38.5445 20.5522 38.5439C19.8802 38.5434 19.2307 38.3017 18.7218 37.863L18.1155 37.3368C9.35433 29.6697 4.91602 22.6441 4.91602 16.1831C4.91602 7.54547 11.9175 0.543945 20.5552 0.543945ZM20.5552 10.168C18.9599 10.168 17.4299 10.8018 16.3019 11.9298C15.1738 13.0578 14.5401 14.5878 14.5401 16.1831C14.5401 17.7784 15.1738 19.3083 16.3019 20.4364C17.4299 21.5644 18.9599 22.1982 20.5552 22.1982C22.1505 22.1982 23.6804 21.5644 24.8085 20.4364C25.9365 19.3083 26.5702 17.7784 26.5702 16.1831C26.5702 14.5878 25.9365 13.0578 24.8085 11.9298C23.6804 10.8018 22.1505 10.168 20.5552 10.168Z"
                    fill="url(#paint0_linear_1448_14701)"
                  />
                  <defs>
                    <linearGradient
                      id="paint0_linear_1448_14701"
                      x1="4.91602"
                      y1="0.543945"
                      x2="45.0587"
                      y2="17.2727"
                      gradientUnits="userSpaceOnUse"
                    >
                      <stop offset="0" stop-color="#FC466B" />
                      <stop offset="1" stop-color="#3F5EFB" />
                    </linearGradient>
                  </defs>
                </svg>
              </div>
              <h3 class="cs-contact_title">Location</h3>
              <p class="cs-contact_text">2118 Thornridge Cir, 35624</p>
            </div>
          </div>
        </div>
        <div class="cs-height_50 cs-height_lg_50"></div>
        <div class="cs-contact_box">
          <div class="cs-section_heading cs-style4">
            <h2 class="cs-section_title">Drop Us Your Message</h2>
            <p class="cs-section_subtitle">
              Freely contact with us anytime. We're available here for you.
            </p>
          </div>
          <div class="cs-height_45 cs-height_lg_45"></div>
          <form
            class="cs-contact_form"
            @submit.prevent="handleSubmit"
            enctype="multipart/form-data"
          >
            <div class="row">
              <div class="col-lg-6">
                <div class="cs-form_field_wrap">
                  <input
                    type="text"
                    class="cs-form_field"
                    placeholder="Full Name"
                    v-model="newContact.fullName"
                    @input="validateFullName"
                    :class="{ 'is-invalid': errors.fullName }"
                  />
                </div>
                <div v-if="errors.fullName" style="color: red">
                  {{ errors.fullName }}
                </div>

                <div class="cs-height_20 cs-height_lg_20"></div>
              </div>

              <div class="col-lg-6">
                <div class="cs-form_field_wrap">
                  <input
                    type="text"
                    class="cs-form_field"
                    placeholder="Your Email"
                    v-model="newContact.email"
                    @input="validateEmail"
                    :class="{ 'is-invalid': errors.email }"
                  />
                </div>
                <div
                  v-if="errors.email"
                  style="color: red"
                  class="error-message"
                >
                  {{ errors.email }}
                </div>
                <div class="cs-height_20 cs-height_lg_20"></div>
              </div>
              <div class="col-lg-12">
                <div class="cs-form_field_wrap">
                  <input
                    type="text"
                    class="cs-form_field"
                    placeholder="Write Subject"
                    v-model="newContact.title"
                    @input="validateTitle"
                    :class="{ 'is-invalid': errors.title }"
                  />
                </div>
                <div
                  v-if="errors.title"
                  style="color: red"
                  class="error-message"
                >
                  {{ errors.title }}
                </div>
                <div class="cs-height_20 cs-height_lg_20"></div>
              </div>
              <div class="col-lg-12">
                <div class="cs-form_field_wrap">
                  <textarea
                    cols="30"
                    rows="5"
                    placeholder="Message..."
                    class="cs-form_field"
                    v-model="newContact.content"
                    @input="validateContent"
                    :class="{ 'is-invalid': errors.content }"
                  ></textarea>
                </div>
                <div
                  v-if="errors.content"
                  style="color: red"
                  class="error-message"
                >
                  {{ errors.content }}
                </div>
                <div class="cs-height_20 cs-height_lg_20"></div>
              </div>
              <div class="col-lg-12">
                <button class="cs-btn cs-style1 cs-btn_lg" type="submit">
                  Send Message
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>
    </div>
  </div>

  <div class="cs-height_70 cs-height_lg_40"></div>
  <footerHome></footerHome>
</template>

<script>
import startHeader from "@/pages/startHeader.vue";
import footerHome from "@/pages/footer.vue";
import ContactService from "@/service/ContactService.js";
import Swal from "sweetalert2";
import { userInfo } from "@/store.js";
export default {
  name: "homeContact",
  components: {
    startHeader,
    footerHome,
  },
  data() {
    return {
      userinfordata: userInfo.value,
      newContact: {
        fullName: "",
        email: "",
        title: "",
        content: "",
      },
      errors: {
        fullName: "",
        email: "",
        title: "",
        content: "",
      },
    };
  },
  created() {
    // Nếu userInfo khác null, gán giá trị mặc định cho fullName
    if (userInfo.value) {
      this.newContact.fullName = userInfo.value.name;

      this.newContact.email = userInfo.value.email;
    }
  },
  methods: {
    validateFullName() {
      //   if (this.newContact.fullName) {
      //     this.errors.fullName = "";
      //   } else {
      //     this.errors.fullName = "Full name is required.";
      //   }
      this.errors.fullName =
        this.newContact.fullName.trim() === "" ? "    Not Blank." : "";
    },
    validateEmail() {
      //   if (!this.newContact.email) {
      //     this.errors.email = "Email is required.";
      //   } else {
      //     this.errors.email = "";
      //   }
      //   this.errors.email =
      //     this.newContact.email.trim() === "" ? "    Not Blank." : "";
      const email = this.newContact.email.trim();

      if (email === "") {
        this.errors.email = "Not Blank.";
      } else {
        // Sử dụng biểu thức chính quy để kiểm tra địa chỉ email hợp lệ
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) {
          this.errors.email = "Invalid email format.";
        } else {
          this.errors.email = "";
        }
      }
    },
    validateTitle() {
      //   if (!this.newContact.title) {
      //     this.errors.title = "Title is required.";
      //   } else {
      //     this.errors.title = "";
      //   }
      this.errors.title =
        this.newContact.title.trim() === "" ? "    Not Blank." : "";
    },
    validateContent() {
      //   if (!this.newContact.content) {
      //     this.errors.content = "Content is required.";
      //   } else {
      //     this.errors.content = "";
      //   }
      this.errors.content =
        this.newContact.content.trim() === "" ? "    Not Blank." : "";
    },
    async handleSubmit() {
      this.validateFullName();
      this.validateEmail();
      this.validateTitle();
      this.validateContent();
      if (
        !this.errors.fullName &&
        !this.errors.email &&
        !this.errors.title &&
        !this.errors.content
      ) {
        try {
            await ContactService.createNewContact(
            this.newContact.fullName,
            this.newContact.email,
            this.newContact.title,
            this.newContact.content,
          );
          // Xử lý sau khi tạo thành công (ví dụ: chuyển hướng hoặc hiển thị thông báo)
       
            Swal.fire({
              icon: "success",
              title: "Contact Created",
              text: "Contact has been created successfully.",
            });
            this.newContact = {
              fullName: "",
              email: "",
              title: "",
              content: "",
            };
        

          console.log("Contact created successfully!");
        } catch (error) {
          Swal.fire({
            icon: "error",
            title: "error",
            text: "Contact has been created successfully.",
          });
          console.error("Error creating contact:", error);
        }
      }
    },
  },
};
</script>

<style></style>
